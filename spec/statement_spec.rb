...
describe Statement do
  ...
  context '#print_statement' do
    it 'prints headings' do
      expect(subject.print_transactions).to eq "#{time.strftime("%d/%m/%Y")} || credit || debit || balance"
    end

    it 'prints statement entry including single credit' do
      ledger.deposit(100.00)
      expect(statement.print_transactions).to eq "#{time.strftime("%d/%m/%Y")} || 100.00 || || 100.00"
    end
    ...
    it 'prints statement entry multiple debits and credits' do
      ...
      expect(statement.print_transactions).to eq "#{time.strftime("%d/%m/%Y")} || || -50.00 || 150.00"
    end
